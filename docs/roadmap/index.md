...
| 基础设施 | Bun workspace 统一 front 包管理 | ✅ |
...
| OpenAPI 3.2.x | ✅ | 按 3.2 语义消费，不按 3.1 猜测处理；从 Java `5.7.0` / Go `v0.8.0` 起发布。见 [OpenAPI 3.2 支持矩阵](../guide/openapi32) |
...
这不承诺 arbitrary custom vocabulary 的执行语义。OpenAPI 3.2.x 契约见
...
Java `5.7.0` 与 Knife4x Go `v0.8.0` 接入 OpenAPI 3.2.x 完整消费。真实 springdoc 生成仍停留在 3.0/3.1。

| 范围 | 状态 | 契约 |
| --- | --- | --- |
| 规范 3.2 夹具消费 | ✅ | React / WebJar / starter 合成入口 / Disk 聚合 / Knife4x SpecURL |
| 真实 springdoc 3.2 生成 | 未做 | 当前 POM 仍为 1.8.0 / 2.8.9 / 3.0.3，只证明 3.0/3.1 生成 |
| 用户矩阵与限制 | ✅ | [OpenAPI 3.2 支持矩阵](../guide/openapi32) |
...
- 不要为了代码优雅牺牲向后兼容
- 不要一次性停掉 OAS2 兼容维护（`front/vue3` 仍然可用）

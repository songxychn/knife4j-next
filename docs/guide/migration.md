...
::: warning 维护模式
`knife4j-openapi2-spring-boot-starter` 处于**兼容维护模式**，不再接收新功能。前端是本仓库 `front/vue3`（Vue 3 + Vite）的构建产物，只接收回归修复与显示层 bug；无法享受 `knife4j-ui-react` 的新特性（SSE 流式响应、allOf/oneOf 渲染、二进制下载修复等）。

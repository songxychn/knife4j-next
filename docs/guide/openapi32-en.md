...
::: info Published
Java `5.7.2` and Knife4x Go `v0.8.1` consume valid OpenAPI 3.2.x documents as described here. This is not a promise that springdoc generates 3.2; current production dependencies still emit 3.0.x / 3.1.x.
:::
...
| OpenAPI 3.0.x | React | Published | Java `5.7.2` / Go `v0.8.1` |
| OpenAPI 3.1.x | React | Published | Java `5.7.2` / Go `v0.8.1` |
...
Unless a row says otherwise, “supported” means the complete OpenAPI 3.2.x feature set in published Java `5.7.2` / Go `v0.8.1`.

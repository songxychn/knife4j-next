...
- **OpenAPI 3.2**：React 主线对合法 3.2 文档按 3.2 语义消费，不降级为 3.1；不能把 springdoc 3.0/3.1 输出改版本号冒充 3.2 生成。详见 [OpenAPI 3.2 支持矩阵](./openapi32)。
...
OpenAPI 3.2 使用独立离线快照，见 [OpenAPI 3.2 支持矩阵](./openapi32)。
...
- OpenAPI 3.1 / 3.2 只有在外部 Schema 资源图完整时才生成语义指纹，且与 3.0 以及彼此的基线隔离；当前只跟踪 `paths` 操作，不跟踪 Webhook 或字段级 diff。3.2 使用 `oas3.2-v1`。
...
OpenAPI 3.2 使用另一套 `oas3.2-v1` 基线，同样要求完整资源图。

...
- **OpenAPI 3.2（未发布）**：`integration/oas32` 源码可按 3.2 语义消费合法文档，不降级为 3.1；已发布 `5.6.0` 不含该能力。详见 [OpenAPI 3.2 支持矩阵](./openapi32)。
...
集成分支上的 OpenAPI 3.2 使用独立离线快照，尚未随 `5.6.0` 发布，见 [OpenAPI 3.2 支持矩阵](./openapi32)。
...
集成分支上的 OpenAPI 3.2 使用另一套 `oas3.2-v1` 基线，同样要求完整资源图；该能力尚未随 `5.6.0` 发布。

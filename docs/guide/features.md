...
- **OpenAPI 3.1**：React 主线对 3.1.x 使用同一加载、JSON Schema 2020-12、调试诊断、导出与变化提示契约；详见 [OpenAPI 3.1 支持与迁移](./openapi31)。
- **OpenAPI 3.2（未发布）**：`integration/oas32` 源码可按 3.2 语义消费合法文档，不降级为 3.1；已发布 `5.6.1` 不含该能力。详见 [OpenAPI 3.2 支持矩阵](./openapi32)。
...
快照入口复用统一的 OpenAPI 3.1.x 版本判断，不按 patch 版本分叉能力。
完整边界见 [OpenAPI 3.1 支持与迁移](./openapi31#产品能力矩阵)。
集成分支上的 OpenAPI 3.2 使用独立离线快照，尚未随 `5.6.1` 发布，见 [OpenAPI 3.2 支持矩阵](./openapi32)。
...
::: tip 调试器请求 baseUrl 优先级（自 5.0.3 起）
...
2. **operation `servers[0].url`** —— 接口级 `servers` 优先于路径和根文档，用于单个接口覆盖调试地址（PR #374）。
3. **path item `servers[0].url`** —— 路径级 `servers` 用于同一路径下多个操作共享覆盖地址（PR #374）。
4. **root `servers[0].url`** —— 文档根级 server URL，包含 context-path；相对路径会基于当前 UI origin 解析（PR #363）。
5. **当前 UI origin** —— 兜底使用 `window.location.origin`，与历史行为一致。

当文档未显式声明协议、且调试地址与当前页面同源时，React UI 会保留当前页面协议，避免 HTTPS 页面错误降级到 HTTP（PR #377）。
...
React UI 在 OpenAPI 3.0.x、3.1.x 以及集成分支上的 3.2.x 中提供以下行为：
...
- OpenAPI 3.1 / 3.2 只有在外部 Schema 资源图完整时才生成语义指纹，且与 3.0 以及彼此的基线隔离；当前只跟踪 `paths` 操作，不跟踪 Webhook 或字段级 diff。3.2 使用 `oas3.2-v1`，属于未发布的集成分支能力。
...
集成分支上的 OpenAPI 3.2 使用另一套 `oas3.2-v1` 基线，同样要求完整资源图；该能力尚未随 `5.6.1` 发布。
Vue3 UI 继续保留 OAS2 兼容实现。详见 [OpenAPI 3.1 支持与迁移](./openapi31) 和 [OpenAPI 3.2 支持矩阵](./openapi32)。
:::

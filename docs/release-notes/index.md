...
- 接口页签右键菜单支持关闭左侧与关闭右侧，并保持固定页签、当前页签回退和持久化顺序语义（PR #664，issue #656）。
- 响应区域可按设置展示状态码概要；单响应仍保持简洁，多响应可显示状态码，并可选择同时展示描述、Schema 类型与媒体类型（PR #665，issue #657）。
- `enableVersion` 默认关闭；启用后按 OpenAPI 来源、应用路径、分组与文档地址隔离本地基线，在侧边栏标记新增和已变化接口，并支持单接口或当前分组全部确认（PR #674，issue #660）。

**本地数据与隐私（React UI）**

- 设置页提供“清理请求缓存”和“重置全部本地数据”两个明确边界的操作，只处理当前 origin 下 Knife4j 已登记的设置、参数、鉴权、页签、OAuth 临时状态、调试缓存与历史，不清理宿主应用数据或浏览器 Cookie（PR #668、#670，issue #659）。
- 重置与持久化写入使用 Web Locks 或安全回退协议协调多页签竞争；无法证明所有权、发布代次或完成清理时安全中止并报告失败，避免旧页签回填或误删新数据（PR #669，issue #659）。
...
- 请求参数、Cookie 会话与 OpenAPI 鉴权拆分为三个独立页面；Cookie 会话配置与手填鉴权凭据限定在当前分组，切组、直达旧标签、文档加载失败和 OAuth2 异步完成时均保持配置隔离。实际 Cookie 按浏览器规则共享，不按分组或端口隔离（PR #627）。

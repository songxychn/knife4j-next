...
Knife4x 已发布版本消费 OpenAPI 3.0.x / 3.1.x / 3.2.x JSON 文档，不生成 spec，不支持 OAS2 / Swagger 2。核心只依赖
标准库 `net/http`；Gin 只是可运行的组合示例，不是库依赖。合法 3.2 JSON 可经 `SpecURL`
交给同一套 React UI；限制见
[OpenAPI 3.2 支持矩阵](../../docs/guide/openapi32.md)。

当前公开版本为 `v0.8.3`，对应仓库 tag `knife4x/go/v0.8.3`：

```bash
go get github.com/songxychn/knife4j-next/knife4x/go@v0.8.3
```

`v0.8.3` 保持 Go 1.22 基线、`Config`、`NewHandler` 与路由语义不变，内嵌 React UI
修复 OAS 3.2 标签 Tooltip 重复名称、改进 Schema 元校验错误定位、普通 JSON 示例默认展示逻辑数据，以及调试 JSON 响应折叠与等价 cURL 排版。OpenAPI 3.2.x 消费从 `v0.8.0`
...
[OpenAPI 3.1 支持与迁移](../../docs/guide/openapi31.md) 与
[OpenAPI 3.2 支持矩阵](../../docs/guide/openapi32.md)。
...
已发布 `v0.8.3` 可加载合法 3.2 JSON。启动后打开

...
当前公开版本为 `v0.8.2`，对应仓库 tag `knife4x/go/v0.8.2`：

```bash
go get github.com/songxychn/knife4j-next/knife4x/go@v0.8.2
```

`v0.8.2` 保持 Go 1.22 基线、`Config`、`NewHandler` 与路由语义不变，内嵌 React UI
修复 OAS 3.2 标签菜单重复名称、首页服务地址明细默认折叠，以及窄窗口首页统计卡片与外壳横向溢出。OpenAPI 3.2.x 消费从 `v0.8.0`
...
已发布 `v0.8.2` 可加载合法 3.2 JSON。启动后打开

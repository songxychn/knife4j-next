...
func TestHandlerServesOpenAPI32JSONThroughEmbeddedUI(t *testing.T) {
	t.Parallel()

	spec, err := os.ReadFile("testdata/openapi-3.2-minimal.json")
	if err != nil {
		t.Fatalf("read OAS 3.2 fixture: %v", err)
	}

	next := http.NewServeMux()
	next.HandleFunc("/openapi.json", func(w http.ResponseWriter, r *http.Request) {
	...
	doc := request(t, got, http.MethodGet, "/doc.html", http.StatusOK)
	config := readInjectedConfig(t, doc.Body.String())
	if config["specUrl"] != "/openapi.json" {
		t.Fatalf("injected specUrl = %q, want /openapi.json", config["specUrl"])
	}

	reference := firstAssetReference(t, doc.Body.String())
	request(t, got, http.MethodGet, "/"+reference, http.StatusOK)

	specResponse := request(t, got, http.MethodGet, "/openapi.json", http.StatusOK)
	body := specResponse.Body.String()
	for _, fragment := range []string{
		`"openapi": "3.2.0"`,
		`"query"`,
		`"COPY"`,
		"not generated by springdoc",
	} {
		if !strings.Contains(body, fragment) {
			t.Fatalf("OpenAPI 3.2 fixture missing %q:\n%s", fragment, body)
		}
	}
}

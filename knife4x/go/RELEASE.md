...
相较于 `knife4x/go/v0.8.3`，本次发布已合入共享 React UI 的 JSON 示例与预览层级折叠，以及请求预览词法保真与空容器折叠（PR #833）。
...
grep -Fq 'knife4x/go/v0.8.4' knife4x/README.md knife4x/go/README.md
grep -Fq 'go@v0.8.4' knife4x/go/MIGRATING_FROM_GIN_SWAGGER.md
...
```bash
git tag -a knife4x/go/v0.8.4 -m "Knife4x Go v0.8.4"
git push origin knife4x/go/v0.8.4
```

不要同时创建根级 `v0.8.4` tag，不要运行 Java Maven Release，不要为本次发布新增
...
编译、Handler 运行和下载内容检查都通过后，才可宣布 Knife4x Go `v0.8.4` 发布完成。

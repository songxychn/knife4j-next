...
相较于 `knife4x/go/v0.8.0`，本次发布已合入共享 React UI 的 OAS 3.1 SchemaEngine
元校验失败重试修复：同一次登记内非法 Schema 元数据保持稳定拒绝，重试不能错误放行（PR #810）。
...
grep -Fq 'knife4x/go/v0.8.1' knife4x/README.md knife4x/go/README.md
grep -Fq 'go@v0.8.1' knife4x/go/MIGRATING_FROM_GIN_SWAGGER.md
...
```bash
git tag -a knife4x/go/v0.8.1 -m "Knife4x Go v0.8.1"
git push origin knife4x/go/v0.8.1
```

不要同时创建根级 `v0.8.1` tag，不要运行 Java Maven Release，不要为本次发布新增
...
编译、Handler 运行和下载内容检查都通过后，才可宣布 Knife4x Go `v0.8.1` 发布完成。

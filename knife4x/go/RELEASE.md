...
相较于 `knife4x/go/v0.8.2`，本次发布已合入共享 React UI 的四笔变更：OAS 3.2 标签 Tooltip 重复名称（PR #826）、OAS 3.2 文档 Schema 元校验错误定位（PR #828）、普通 JSON 示例默认展示逻辑数据（PR #827），以及调试 JSON 响应折叠与等价 cURL 排版（PR #830）。
...
grep -Fq 'knife4x/go/v0.8.3' knife4x/README.md knife4x/go/README.md
grep -Fq 'go@v0.8.3' knife4x/go/MIGRATING_FROM_GIN_SWAGGER.md
...
```bash
git tag -a knife4x/go/v0.8.3 -m "Knife4x Go v0.8.3"
git push origin knife4x/go/v0.8.3
```

不要同时创建根级 `v0.8.3` tag，不要运行 Java Maven Release，不要为本次发布新增
...
编译、Handler 运行和下载内容检查都通过后，才可宣布 Knife4x Go `v0.8.3` 发布完成。

...
| 后端实现，新 React 前端已读取部分默认值 | `knife4j.setting.language`、`enable-debug`、`enable-search`、`enable-open-api`、`enable-host`、`enable-group`、`enable-footer`、`swagger-model-name` 等 | 这些是 UI 开关，不替代后端安全控制；本地设置面板选择优先 |

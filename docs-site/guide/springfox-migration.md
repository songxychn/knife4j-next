...
### `@ApiImplicitParam.allowableValues` → `@Schema`

`@ApiImplicitParam` 的 `allowableValues` 属性在 OpenAPI3 中通过 `@Parameter` 内嵌的 `@Schema` 表达。更多 `@ApiImplicitParam` → `@Parameter` 的通用替换示例见下方[注解替换示例](#注解替换示例)。
...
@GetMapping("/users")
public List<UserVO> listUsers(@RequestParam String status) {
...
如果状态值已有对应的 Java enum，直接用 `@Schema(implementation = UserStatus.class)` 更简洁，springdoc 会自动提取枚举成员：
...
`allowableValues = "range[1, 100]"` 在 OpenAPI3 中用 `@Schema` 的 `minimum` / `maximum` 属性表达：
...
::: tip 半开区间
Swagger2 的 `range(1, 100)`（圆括号）表示不含端点。OAS 3.0（springdoc-openapi 1.x 默认输出）中 `exclusiveMinimum` / `exclusiveMaximum` 是布尔值；OAS 3.1 中它们改为数值（springdoc-openapi 2.x 默认仍输出 OAS 3.0，需额外配置才启用 3.1）。最简单的做法是在 `description` 中说明边界是否包含。
:::

...
`@ApiImplicitParam` 的 `allowableValues` 属性在 OpenAPI3 中通过 `@Parameter` 内嵌的 `@Schema` 表达。
...
::: tip 半开区间
Swagger2 的 `range(1, 100)`（圆括号）表示不含端点。OpenAPI3 没有直接对应，可用 `exclusiveMinimum` / `exclusiveMaximum`（布尔值，springdoc 2.x 支持）或在 `description` 中说明。
:::

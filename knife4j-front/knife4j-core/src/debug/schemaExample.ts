...
function buildExampleInternal(
  ...
  if (
    ...
    if (props && Object.keys(props).length > 0) {
      ...
      const addSchema = resolved.additionalProperties as Record<
        string,
        unknown
      >;
      result["additionalProp1"] = buildExampleInternal(
        addSchema,
        childCtx(ctxWithRef),
      );
    }
...
function buildFieldTreeInternal(
  ...
  if (type === "array") {
    ...
    if (items && ctx.depth + 1 < ctx.maxDepth) {
      const itemNode = buildSingleFieldNode(
        "items",
        items,
        false,
        childCtx(pushRefIfAny(ctx, ref)),
      );
...
function objectToFieldNodes(
  ...
  for (const [name, propSchema] of Object.entries(props)) {
    nodes.push(
      buildSingleFieldNode(
        name,
        propSchema,
        requiredSet.has(name),
        pushRefIfAny(ctx, parentRef),
      ),
    );
  }
...
function buildSingleFieldNode(
  ...
  if (
    ...
    ctx.refChain.includes(rawSchema.$ref)
  ) {
    ...
    const circularOwnDesc =
      typeof rawSchema.description === "string"
        ? rawSchema.description
        : undefined;
    const circularRefDesc =
      circularTarget && typeof circularTarget.description === "string"
        ? circularTarget.description
        : undefined;
    return {
      name,
      type: "object",
      ...
      refDescription:
        circularOwnDesc &&
        circularRefDesc &&
        circularRefDesc !== circularOwnDesc
          ? circularRefDesc
          : undefined,
      refTitle:
        circularTarget && typeof circularTarget.title === "string"
          ? circularTarget.title
          : undefined,
    };
  }
  ...
  const refDescription =
    ...
    refTargetDescription !== ownDescription
      ? refTargetDescription
      : undefined;
  const refTitle = ref
  ...
  const node: SchemaFieldNode = {
    ...
    enum: Array.isArray(resolved.enum) ? resolved.enum : undefined,
    minLength:
      typeof resolved.minLength === "number" ? resolved.minLength : undefined,
    maxLength:
      typeof resolved.maxLength === "number" ? resolved.maxLength : undefined,
    minimum:
      typeof resolved.minimum === "number" ? resolved.minimum : undefined,
    maximum:
      typeof resolved.maximum === "number" ? resolved.maximum : undefined,
    pattern:
      typeof resolved.pattern === "string" ? resolved.pattern : undefined,
    readOnly:
      typeof resolved.readOnly === "boolean" ? resolved.readOnly : undefined,
    writeOnly:
      typeof resolved.writeOnly === "boolean" ? resolved.writeOnly : undefined,
    deprecated:
      typeof resolved.deprecated === "boolean"
        ? resolved.deprecated
        : undefined,
    refName: refToName(ref),
  };
  ...
  if (node.type === "object") {
    ...
    if (items) {
      ...
      if (
        ...
        deeperCtx.refChain.includes(items.$ref)
      ) {
        node.children = [
          {
            name: "items",

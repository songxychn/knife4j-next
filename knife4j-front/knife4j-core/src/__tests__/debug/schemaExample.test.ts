...
describe('TASK-114: allOf/oneOf/anyOf schema inheritance', () => {
  ...
  // 6. 非空 properties + additionalProperties 共存时，additionalProperties 不出现在结果中
  test('properties take priority over additionalProperties in example', () => {
    const result = buildSchemaExample({ $ref: '#/components/schemas/PropsWithAdditional' }, ctx()) as Record<
      string,
      unknown
    >;
    // properties 展开为 name/age，additionalProperties 不额外产生字段
    expect(result).toEqual({ name: 'string', age: 0 });
    expect(result).not.toHaveProperty('additionalProp1');
  });

  test('properties take priority over additionalProperties in field tree', () => {
    const nodes = buildSchemaFieldTree({ $ref: '#/components/schemas/PropsWithAdditional' }, ctx());
    // 只展示 properties 中的字段，不出现 * 伪字段
    const names = nodes.map((n) => n.name);
    expect(names).toContain('name');

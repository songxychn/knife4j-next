...
describe("replacePathParams", () => {
  ...
  test("encodes special characters", () => {
    expect(replacePathParams("/files/{path}", { path: "a/b c" })).toBe(
      "/files/a%2Fb%20c",
    );
  });

  test("ignores empty param name", () => {
    expect(replacePathParams("/users/{id}", { "": "x", id: "1" })).toBe(
      "/users/1",
    );
  });
  ...
  test("handles RFC 6570 {+name} style", () => {
    expect(replacePathParams("/files/{+path}", { path: "a/b" })).toBe(
      "/files/a%2Fb",
    );
  });
});
...
describe("validateRequired", () => {
  const model: OperationDebugModel = {
    ...
    headerParams: [
      { name: "X-Auth", in: "header", required: true, type: "string" },
    ],
    cookieParams: [],
    bodyContents: [
      { mediaType: "application/json", category: "json", schema: {} },
    ],
    bodyRequired: true,
  };
  ...
  test("multipart body: ok when at least one file uploaded", () => {
    const multipartModel: OperationDebugModel = {
      ...
      cookieParams: [],
      bodyContents: [
        { mediaType: "multipart/form-data", category: "multipart", schema: {} },
      ],
      bodyRequired: true,
    };
    const form: DebugFormValues = {
...
describe("buildRequest", () => {
  const debugModel: OperationDebugModel = {
    ...
    headerParams: [
      { name: "X-Token", in: "header", required: false, type: "string" },
    ],
    cookieParams: [],
    bodyContents: [
      { mediaType: "application/json", category: "json", schema: {} },
  ...
  test("merges global params (global overrides user values, issue #275)", () => {
    const result = buildRequest({
      baseUrl: "http://localhost:8080",
      path: "/users/{id}",
      method: "GET",
    ...
    // global params override user values (globalParameters > Authorize > 用户表单)
    expect(result.query["verbose"]).toBe("false");
...
describe("buildRequest sourceMap", () => {
  ...
  test("global overrides interface and auth in sourceMap (issue #275)", () => {
    const result = buildRequest({
      baseUrl: "http://localhost",
      path: "/api/test",
      method: "GET",
    ...
    // Global overrides both auth and interface for Authorization (global has highest priority)
    // Auth sets Authorization=auth, global sets X-Global=global, interface sets Authorization=interface
    // Final: global wins over auth wins over interface
    // Authorization: interface sets it, auth overrides it, but global doesn't set Authorization here
    // So Authorization source = auth (auth overrides interface)
    expect(result.sourceMap!.headers["Authorization"]).toBe("auth");
    // gq: interface sets '2', global sets '1' → global wins
    expect(result.sourceMap!.query["gq"]).toBe("global");
    // Global header should still be 'global'
    expect(result.sourceMap!.headers["X-Global"]).toBe("global");
    // Actual merged values
    expect(result.headers["Authorization"]).toBe("Bearer mytoken");
    expect(result.query["gq"]).toBe("1");
  });

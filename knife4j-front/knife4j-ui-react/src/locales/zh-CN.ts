const zhCN = {
  ...
  "apiDoc.notFound.desc":
    "当前路由没有匹配到 OpenAPI operation，请重新从左侧接口列表打开。",
  ...
  "apiDebug.preview.bodyMultipart":
    "请求体（multipart/form-data，文件字段需在真实发送时追加）",
  ...
  "officeDoc.desc":
    "将当前接口文档导出为可下载的离线文件，支持 HTML、Word（.docx）和 Word（.doc）格式。",
  "officeDoc.alert.mockData":
    "当前使用 Mock 数据或文档未加载，导出内容可能不完整。",
  ...
  // Settings page
  "settings.tip":
    "启用个性化配置后，接口 Tab 标签需关闭后重新打开或者刷新当前页面",
  "settings.enableRequestCache":
    "开启请求参数缓存（Debug 调试栏自动缓存上次填写的参数值）",
  "settings.enableDynamicParameter":
    "开启动态参数（调试时请求参数支持动态注入）",
  "settings.enableFilterMultipartApis":
    "过滤接口类型（针对 RequestMapping 未指定类型的接口，只展示指定类型）",
  ...
  "settings.tagsSorter.desc":
    "默认跟随 springdoc.swagger-ui.tags-sorter；选择后覆盖后端配置。",
  ...
  // Footer 设置
  "settings.footerEnabled": "显示底部 Footer",
  "settings.footerCustomContent.placeholder": "留空则显示默认版权文本",
  "settings.footerCustomContent.desc":
    "自定义 Footer 内容（留空时显示默认版权文本）",

...
function buildConstraintTooltip(node: SchemaFieldNode): string | undefined {
  const lines: string[] = [];
  ...
  if (node.enum !== undefined && node.enum.length > 0)
    lines.push(`enum: ${node.enum.join(", ")}`);
...
export function SchemaTypeLink({ node }: SchemaTypeLinkProps) {
  ...
  const constraintText = buildConstraintTooltip(node);
  ...
  if (!constraintText) return tag;

  return (
    <Tooltip
      title={<span style={{ whiteSpace: "pre-line" }}>{constraintText}</span>}
      placement="top"
    >
      {tag}
    </Tooltip>

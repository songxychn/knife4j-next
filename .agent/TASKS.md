...
> ⚠️ **本文件已于 2026-04-26 冻结，不再更新。** 新任务通过 GitHub Issues 管理，使用 `agent-task` + `status:*` + `area:*` label。查看当前任务：`gh issue list --label agent-task` 或 `./scripts/agent-status.sh`。以下内容仅作历史参考。
...
### TASK-025
status: done
merged_into: master@2026-04-25T13:27:38Z
area: ui-react
title: i18n 中英文切换

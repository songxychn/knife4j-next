...
- 默认遵守 `User/Maintainer -> Coordinator -> Worker -> Reviewer -> Worker(如需返工) -> Coordinator`。
...
- 状态变更命令：
  ...
  - 完成（**仅在 reviewer 门禁通过且 PR CI 全绿后**）：`gh issue edit <N> --remove-label status:in-progress --add-label status:review`
...
委派规则：
- 对任何非例外范围的源码改动、重文件阅读、有边界实现或独立探索使用 worker。
- 除纯 label/issue/PR 元数据操作外，agent 产出的 PR 默认需要 reviewer；当实现被委派、Java 兼容性行为变化、多模块变更，或验证曾失败后才通过时，在 PR 前必须使用 reviewer。
- 跳过 worker/reviewer 时，必须记录例外原因、替代验证和剩余风险。
- 不要创建递归 agent 层级。
...
- 状态变化
- 验证结果
- worker/reviewer handoff 摘要或跳过原因
- 下一步
- blocker，如有
```

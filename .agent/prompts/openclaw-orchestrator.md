...
11. coordinator 根据 worker handoff 结果，在 issue 评论里记录进度（不再修改 .agent/TASKS.md 或 .agent/PROGRESS.md）。
12. 任务可审查时创建或更新 PR。创建 PR 后：
    - 立即通过 Telegram 通知维护者：
      `openclaw message send --account knife4j-next-bot --channel telegram --target 6358501334 --message "[TASK-XXX] PR #N 已创建：<PR 链接>"`
    - **不要在此等待 CI**。issue 保持 `status:in-progress`，退出本次运行。
    - CI 状态由 OpenClaw 心跳在下次轮询时检查（见心跳流程第 2 步）。
...
- 本地验证 ≠ CI 验证。CI 状态由心跳轮询，不在 CC 内阻塞等待。

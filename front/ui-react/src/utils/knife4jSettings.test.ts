...
describe('knife4j setting extraction', () => {
  ...
  it.each([true, false])('reads boolean API version prompt setting %s', (enableVersion) => {
    const doc = baseDoc({
      'x-openapi': {
        'x-setting': { enableVersion },
      },
    });

    expect(extractKnife4jSettings(doc)).toEqual({ enableVersion });
  });

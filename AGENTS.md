...
两个合理方案会产生不同外部行为、需要跨模块架构取舍、失败超出任务范围、影响 release / 兼容承诺或维护者意图不明时，先自行调查；仍需维护者决策时，说明已有证据和必要缺口，只暂停依赖该决策的步骤，继续其他已授权工作。

Java 兼容、多模块、CI / 构建 / 发布逻辑，以及原因未查明的门禁失败、仅靠重试转绿或验证标准改变属于高风险，独立 reviewer 须重点核对真实 diff 与证据，维护者决定是否追加人工审。预期的 bug 复现失败、修复后同一证据转绿，不因这一过程本身升级风险。
...
| `status:review` | 本地验证 + 独立审查 + PR CI 已过，等人 merge |
...
| 跨区域 | 受影响区域标准脚本的并集，包含生成物与消费方影响；见 RUNBOOK |
| 全区域变更 / 整体验收 | `./tools/test-all.sh` |
